...
const HandleBusinessHero = () => {
  ...
  const fetchHeroData = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await api.get('/admin/business-hero');
      setHeroData(response.data.data);
      console.log('Fetched business hero data:', response.data.data);
      ...
      setError('Failed to fetch business hero section data. Please try again.');
      console.error('Error fetching business hero data:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = heroData?.id 
        ? `/admin/business-hero/${heroData.id}`
        : '/admin/business-hero';
      ...
      if (response.data.success) {
        setSuccess(response.data.message || 'Business hero section saved successfully!');
        // Refresh data
        fetchHeroData();
      } else {
        setError(response.data.message || 'Failed to save business hero section.');
      }
    } catch (err) {
      console.error('Error saving business hero section:', err.response?.data || err);
      ...
      if (err.response?.data?.errors) {
        ...
        const errorMessage = err.response?.data?.message || 
                          ...
                          'Failed to save business hero section. Please try again.';
        setError(errorMessage);
      }
    } finally {
      setSaving(false);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 md:p-6">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Business Page - Hero Section</h1>
          <p className="text-gray-600 mt-2">
            Manage the hero section for the Business page
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
          <div className="mb-8">
            <h2 className="text-xl font-semibold text-gray-900">
              Business Hero Configuration
            </h2>
            <p className="text-gray-600 text-sm mt-1">
              {heroData?.id 
                ? `Editing existing business hero section (ID: ${heroData.id})` 
                : 'Create new business hero section'}
            </p>
          </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent bg-white text-gray-900 placeholder-gray-400"
                    placeholder="e.g., Powering Smart Enterprises"
                  />
                </div>
...
export default HandleBusinessHero;